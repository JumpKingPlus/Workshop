using TASStudio.Controls;
using TASStudio.Entities;
...
namespace TASStudio {
	public partial class Studio : Form
	{
		...
		private const string RegKey = "HKEY_CURRENT_USER\\SOFTWARE\\JumpKingStudio\\Form";
		...
		public void EnableStudio(bool hooked) {
			if (hooked) {
				string fileName = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "JumpKing.tas");
		...
		public void UpdateValues() {
			if (this.InvokeRequired) {
				...
				string tas = JumpKingTAS.Manager.CurrentStatus;
		...
		private void UpdateStatusBar() {
			lblStatus.Text = "(" + (currentFrame > 0 ? currentFrame + "/" : "") + totalFrames + ") " + JumpKingTAS.Manager.PlayerStatus;
